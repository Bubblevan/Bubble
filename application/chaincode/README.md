直接在test-network上打包chaincode这段链码，可极大程度的参考lab7。
...
peer lifecycle chaincode package fabasg.tar.gz --path ~/ASG-Ticket/application/chaincode --lang golang --label fabasg_1
<这里替换成自己的路径>
```
...
export CC_PACKAGE_ID=<这里自己的放PACKAGE_ID>
```
...
```SQL
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile ${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C mychannel -n fabasg --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt -c '{"function":"QueryTicket","Args":["TICKET0"]}'
```